...
import { AppState, AppActions, FolderNode, PhotoMetadata } from '@/types';
...
export const useAppStore = create<AppStore>()(
  persist(
    (set, get) => ({
      ...
      setRequestPageSize: (size: number) =>
        set({ requestPageSize: Math.max(10, Math.min(500, Math.floor(size))) }),
      ...
      setFolderTree: (tree: FolderNode[] | null) => set({ folderTree: tree }),
      ...
      saveAudioTime: (audioId: string, time: number) =>
        set((state) => ({
          podcastMode: {
            ...state.podcastMode,
            audioTimeTracking: {
      ...
      openAudioPlayer: (audio: PhotoMetadata, playlist?: PhotoMetadata[]) =>
        set((state) => ({
          audioPlayer: {
            ...state.audioPlayer,
            isOpen: true,
      ...
      setAudioPlayerState: (updates: Partial<AppState['audioPlayer']>) =>
        set((state) => ({
          audioPlayer: {
            ...state.audioPlayer,
            ...updates,
      ...
      setVisualizerSettings: (settings: Partial<AppState['visualizerSettings']>) =>
        set((state) => ({
          visualizerSettings: {
    ...
    }),
    {
      ...
      partialize: (state) => ({
        ...
        // Query-owned library data is intentionally not persisted here.
      }),
    },
  ),
);
