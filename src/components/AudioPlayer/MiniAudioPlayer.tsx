...
export const MiniAudioPlayer: React.FC<MiniAudioPlayerProps> = ({ className = '' }) => {
  const { audioPlayer, playNextAudio, playPreviousAudio, setMiniPlayer, closeAudioPlayer } = useAppStore();
  ...
  return (
    <div
      className={`bg-gray-900 text-white flex items-center px-4 py-2 rounded-lg shadow gap-4 w-full ${className}`}
      style={{ minHeight: 40, maxWidth: '100%' }}
    >
      <div className="flex flex-1 min-w-0 items-center gap-3">
        <div className="flex-1 min-w-0">
          <div className="truncate font-semibold text-base max-w-full" title={currentAudio.name}>
            {currentAudio.name}
          </div>
