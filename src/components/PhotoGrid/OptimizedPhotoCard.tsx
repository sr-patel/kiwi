...
export const OptimizedPhotoCard: React.FC<OptimizedPhotoCardProps> = ({
  photo,
  size,
  isSelected,
  onSelect,
  ...
  // Use lazy loading
  const { ref, isVisible, hasLoaded } = useLazyLoading({
    threshold: 0.1,
    rootMargin: '100px',
  });
  ...
  React.useEffect(() => {
    if (isVisible && !imageSrc && !imageError) {
      ...
      console.log('Loading image:', { photoId: photo.id, photoName: photo.name, thumbnailUrl });
      setImageSrc(thumbnailUrl);
    }
  }, [isVisible, imageSrc, imageError, photo.id, photo.name]);
  ...
  const handleImageLoad = useCallback(() => {
    console.log('Image loaded successfully:', { photoId: photo.id, photoName: photo.name });
    setIsLoading(false);
  }, [photo.id, photo.name]);
