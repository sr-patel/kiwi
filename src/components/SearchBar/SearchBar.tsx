...
export const SearchBar: React.FC<SearchBarProps> = ({ className = '' }) => {
  const { searchQuery, setSearchQuery, filters, setFilters, accentColor, enableColorIntegration } =
    useAppStore();
  ...
  const updateSearchQuery = (content: string, tags: string[]) => {
    ...
    const searchParts = [...contentParts, ...tagParts];
  ...
  return (
    <div className={`flex gap-2 ${className}`}>
      ...
      <div className="flex-1 min-w-0" data-tag-selector>
        <TagSelector selectedTags={selectedTags} onTagsChange={handleTagsChange} placeholder="Search tags" />
      </div>
    </div>
  );
};
